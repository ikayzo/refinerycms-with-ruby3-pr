    ...
    describe "#truncate" do
      ...
      context "when preserve_html_tags option is present" do
        it "preserve html tags when truncating text" do
          expect(helper.truncate("<p>Turducken frankfurter ham hock bacon</p>",
                          :preserve_html_tags => true)).to eq("<p>Turducken frankfurter ham...</p>")
        end
      end

      context "when preserve_html_tags option is not present" do
        it "falls back to original truncate method" do
          expect(helper.truncate("<p>Turducken frankfurter ham hock bacon</p>").html_safe).to 
