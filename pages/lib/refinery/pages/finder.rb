  ...
  module Pages
    class Find
      def self.by_path(path)
        FindByPath.new(path).find
      end

      def self.by_path_or_id(path, id)
        FindByPathOrId.new(path, id).find
      end

      def self.by_title(title)
        FindByTitle.new(title).find
      end

      def self.by_slug(slug, conditions={})
        FindBySlug.new(slug, conditions).find
      end

      def self.with_globalize(conditions = {})
        Find.new(conditions).find
      end
    ...
    class FindByPath
      ...
      def find
        if slugs_scoped_by_parent?
          FindByScopedPath.new(path).find
        else
          FindByUnscopedPath.new(path).find
        end
      end
      ...
      def by_slug(slug_path, conditions = {})
        Find.by_slug(slug_path, conditions)
      end
    end

    class FindByScopedPath < FindByPath
      def find
    ...
    class FindByUnscopedPath < FindByPath
      def find
        by_slug(path).first
      end
    end

    class FindByPathOrId
